...
## Unlock PDF API Setup

The Unlock PDF tool now relies on the built-in Next.js API route located at `app/api/unlock/route.ts`, which invokes `qpdf` to remove the password from a document.

### Local development
1. Install [qpdf](https://qpdf.sourceforge.io/) (or use the bundled binary at `backend/bin/qpdf`):
   - macOS: `brew install qpdf`
   - Ubuntu: `sudo apt-get install qpdf`
   - Windows: [Download from qpdf releases](https://github.com/qpdf/qpdf/releases)
2. Make sure the `qpdf` binary is on your `PATH`. If it is installed in a custom location, set `QPDF_BINARY_PATH=/absolute/path/to/qpdf` in `.env.local` (e.g., `QPDF_BINARY_PATH=backend/bin/qpdf`).
3. Start the app with `npm run dev`. The `/api/unlock` route will handle unlock requests automatically when you use the Unlock tool in the UI.

### Deploying to Vercel or other serverless hosts
- Add a Linux-compatible `qpdf` binary to the repository (for example inside `backend/bin/qpdf`) and commit it so that Vercel can execute it at runtime. The currently bundled binary reflects the local macOS build—replace it with a Linux build prior to deployment.
- Set the `QPDF_BINARY_PATH` environment variable in the Vercel dashboard to point to that binary (e.g., `/var/task/backend/bin/qpdf`).
- The Unlock PDF tool requires the Node.js runtime; it is not available in a purely static export generated by `next export`.

The route places temporary files in `/tmp`, deletes them after each request, and streams the unlocked PDF back to the browser. Use HTTPS in production for maximum privacy.
