...
import { LockOpenIcon, ArrowDownTrayIcon, TrashIcon, EyeIcon, EyeSlashIcon } from '@heroicons/react/24/outline'
...
export default function UnlockPdfPage() {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      setProcessing({ progress: 50, currentStep: 'Attempting to unlock...' })
      
      // Try to load the PDF with the password
      const pdf = await PDFDocument.load(pdfBytes)
      
      setProcessing({ progress: 75, currentStep: 'Generating unlocked PDF...' })
      const unlockedBytes = await pdf.save()
      
      setProcessing({ progress: 100, currentStep: 'Complete!' })
      setResult(new Blob([unlockedBytes], { type: 'application/pdf' }))
    ...
    <div className="min-h-screen bg-gradient-to-br from-white to-yellow-50 flex flex-col items-center justify-center py-12 px-4">
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl p-10 flex flex-col items-center">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-lg text-gray-600 text-center">Remove password protection from your PDF files.</p>
        </div>
        ...
        {files.length > 0 && (
          <div className="w-full mb-6">
            ...
            <p className="text-xs text-gray-500 mt-1">Enter the password that protects this PDF</p>
          </div>
        )}
